-- The EULA acceptance gate (eula_gate_screen.dart, FeatureFlags.requireEulaAcceptance,
-- EulaService) has been removed from the client entirely — Terms/Privacy are now
-- surfaced as informational links on the welcome/email-input/login screens instead
-- of a blocking acceptance step. Nothing writes a consent_records row keyed to
-- auth.uid() anymore, so the hard EULA check added in
-- enforce_eula_consent_in_rpc_onboard_profile would now reject every new
-- registration. Revert to just the ownership check (auth.uid() = p_user_id),
-- which remains a valid, independent fix.
