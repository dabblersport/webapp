...
class CircleRepositoryImpl implements CircleRepository {
  ...
  Future<Result<List<CircleContact>, Failure>> circleList() async {
    try {
      final rows = await _db.rpc(SupabaseConfig.rpcCircleListFn);
      if (rows is List) {
        final list = rows
  ...
  Future<Result<List<Map<String, dynamic>>, Failure>> circleView({
    ...
    try {
      final dynamic q = limit != null && offset != null
          ? _db.from(SupabaseConfig.vCircleTable).select().range(offset, offset + limit - 1)
          : _db.from(SupabaseConfig.vCircleTable).select();
      final rows = await q;
  ...
  Future<Result<List<Map<String, dynamic>>, Failure>> circleFeed({
    ...
    try {
      final rows = await _db
          .from(SupabaseConfig.vFeedCircleTable)
          .select()
  ...
  Stream<Result<List<Map<String, dynamic>>, Failure>> circleFeedStream({
    ...
    try {
      ...
      final edgesStream = _db.from(SupabaseConfig.friendEdgesTable).stream(primaryKey: ['id']);
  ...
  Future<Result<List<Map<String, dynamic>>, Failure>> squadCards({
    ...
    try {
      dynamic q = _db.from(SupabaseConfig.vSquadCardTable).select();
      if (squadId != null) {
  ...
  Future<Result<List<Map<String, dynamic>>, Failure>> squadDetail(
    ...
    try {
      final rows = await _db.from(SupabaseConfig.vSquadDetailTable).select().eq('id', squadId);
      return Ok(rows.cast<Map<String, dynamic>>());
    } catch (e) {
      return Err(svc.mapPostgrestError(e));
    }
